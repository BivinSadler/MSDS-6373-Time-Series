...
fit = arima(CM$cmort,order = c(phi$p,0,phi$q), seasonal = list(order = c(0,1,0), period = 52), xreg = cbind(CM$temp, CM$part, CM$Week))
...
fit = arima(CMsmall$cmort,order = c(phi$p,0,0), seasonal = list(order = c(0,1,0), period = 52), xreg = cbind(CMsmall$temp, CMsmall$part, CMsmall$Week))
...
#load the forecasted Part and Temp in a data frame
next20 = data.frame(temp1 = predsTemp$f1, part7 = predsPart$f, Week = seq(509,528,1), FWeek = as.factor(seq(509,528,1)%%52))
#get predictions
predsCMort = predict(fit,newxreg = next20) #creates error because of factor
